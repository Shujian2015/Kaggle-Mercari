...
def main():
    ...
    try:
        predsTF = tf_method(); gc.collect()
        predsFM, predsF = wordbatch_algo()
        preds = (np.expm1(predsF) * 0.10 + np.expm1(predsTF) * 0.60 + np.expm1(predsFM) * 0.30)
    except:
        preds = np.array([26.7] * n_test).reshape(-1, 1)
