...
def wordbatch_algo():
    ...
    # ---------------------------------------
    # FTRL model
    model2 = FTRL(alpha=0.01, beta=0.1, L1=0.00001, L2=1.0, D=train_X.shape[1], iters=40, inv_link="identity", threads=1)
    # del X; gc.collect()
    ...
    for i in range(TF_EPOCH):
        ...
        for idx in batches:
            feed_dict = {
                ...
                place_desc_mean_len_words: X_desc_mean_len_words[idx], 
                place_desc_mean_upper: X_desc_mean_upper[idx], 
                place_desc_mean_numbers: X_desc_mean_numbers[idx], 
                place_name_mean_len_words: X_name_mean_len_words[idx], 
                place_name_mean_upper: X_name_mean_upper[idx], 
                place_name_mean_numbers: X_name_mean_numbers[idx],          
            }
    ...
    for df_test in load_test():
        ...
        for idx in batches:
            feed_dict = {
                ...
                place_desc_mean_len_words: X_desc_mean_len_words_test[idx], 
                place_desc_mean_upper: X_desc_mean_upper_test[idx], 
                place_desc_mean_numbers: X_desc_mean_numbers_test[idx], 
                place_name_mean_len_words: X_name_mean_len_words_test[idx], 
                place_name_mean_upper: X_name_mean_upper_test[idx], 
                place_name_mean_numbers: X_name_mean_numbers_test[idx],                   
            }
...
def main():
    ...
    preds = (np.expm1(predsF) * 0.10 + np.expm1(predsTF) * 0.50 + np.expm1(predsFM) * 0.40)
